package by.itacademy.news_management.controller;
...
import by.itacademy.news_management.service.NewsService;
import by.itacademy.news_management.service.ServiceException;
...
public class NewsController {
	 ...
	 *
	 * <p>
	 * In case new News - shows empty form to fill. In case updating News - shows
	 * filled form with existing data from DB.
	 *
	 * <p>
	 * KEY_CURRENT_PAGE session parameter uses for localization. In case changing
	 * localization while filling new news or updating existing news form return to
	 * the same page or other, depends of algorithm
	 */
	 ...
	 *
	 * <p>
	 * Creates Session parameter KEY_RESULT_OPERATION with result operation message.
	 *
	 * <p>
	...
	public String createNews(@ModelAttribute("news") News news, HttpSession session) {
		...
		try {
			...
			if (!ParameterSession.RESULT_OPERATION_MSG_UPDATE_SUCCESS
					.equals(session.getAttribute(ParameterSession.KEY_RESULT_OPERATION))) {
				session.setAttribute(ParameterSession.KEY_RESULT_OPERATION,
			...
			session.setAttribute(ParameterSession.KEY_RESULT_OPERATION,
					ParameterSession.RESULT_OPERATION_MSG_CREATE_UPDATE_FAILED);
	 ...
	 *
	 * <p>
	 * Session parameter KEY_CURRENT_PAGE changes if delete operation executes form
	 * page "/findNews" so after successfully deletion redirect to main page,
	 * otherwise KEY_CURRENT_PAGE keeps "/findAllNews" page and redirects back to
	 * "/findAllNews" after successfully deletion.
	 *
	 * @param id      - identifies News
	 * @param session - keeps page which redirect after successfully operation
	 *                result.
	 */
	...
	public String deleteNews(@RequestParam("id") int id, HttpSession session,
		...
		try {
			...
			session.setAttribute(ParameterSession.KEY_RESULT_OPERATION,
					ParameterSession.RESULT_OPERATION_MSG_DELETE_SUCCESS);
		} catch (ServiceException e) {
	...
	/**
	 * Deletes group News from "/findAllNews" page. If checkboxes not selected,
	 * delete nothing.
	 */
	 ...
	 * @param session -keeps current page with id in case localization changes and
	 *                the same page should be displayed again.
	 */
	 ...
	 *
	 * <p>
	 * Session parameter KEY_CURRENT_PAGE needs if changes Locale, the same page
	 * should be displayed.
	 *
	 * @param id      - identifies News
	 ...
	 * <p>
	 * Session parameter KEY_CURRENT_PAGE uses to return back to page from which
	 ...
	 * @param session - uses to know what is current page, in case changing
	 *                localization and stay at the same page.
	 */
	...
	public String updateNews(@RequestParam("id") int id, Model model, HttpSession session) {
		...
		try {
			...
			session.setAttribute(ParameterSession.KEY_RESULT_OPERATION,
					ParameterSession.RESULT_OPERATION_MSG_UPDATE_SUCCESS);
		} catch (ServiceException e) {
			logger.error("Error finding news by id / " + e);
			return "redirect:/news/error";
		}
	...
	/**
	 * Defines what page redirect after operation. If operation executed from
	 * '/findAllNews' - stay at the same page, otherwise redirect to main page.
	 *
	 */
