package by.itacademy.news_management.service.impl;
...
import by.itacademy.news_management.dao.DAOException;
import by.itacademy.news_management.dao.NewsDAO;
...
import by.itacademy.news_management.service.validation.NewsValidation;
...
public class NewsServiceImpl implements NewsService {
	...
	public void createNews(News news) throws ServiceException {
		...
		if (NewsValidation.isFormEmptyData(news)) {
			logger.error("Validation isEmptyData FAILED / Some of the fields: 'title', 'brief', 'content' are empty");
			throw new ServiceException(
					"Validation isEmptyData FAILED / Some of the fields: 'title', 'brief', 'content' are empty");
		}

		else if (NewsValidation.isFormFieldOverLength(news)) {
			logger.error(
					"Validation isFormLengthNotCorrect  FAILED / Some of the fields: 'title', 'brief', 'content' are over length");
			throw new ServiceException(
					"Validation isFormLengthNotCorrect  FAILED / Some of the fields: 'title', 'brief', 'content' are over length");
		}
	...
	/**
	 * Finds News by id from DB news == null, if News id is not unique in DB, it
	 * must be unique.
	 *
	 * @param id
	 * @return news
